"""Thin synchronous client for the Airbnb Revenue, Occupancy & ROI Analyzer Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, AirbnbRevenueAnalyzerError

ACTOR_ID = "apivault_labs~airbnb-revenue-occupancy-roi-analyzer"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class AirbnbRevenueAnalyzerClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "airbnb-revenue-analyzer-python/0.1.0",
        })
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise AirbnbRevenueAnalyzerError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise AirbnbRevenueAnalyzerError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise AirbnbRevenueAnalyzerError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
